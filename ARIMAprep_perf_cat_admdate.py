...
#this file uses the hospital strain dates of the admission date.
...
#do the above fr the admission date rather than transfer date
all_transfers['dt_adm'] = all_transfers['dt_adm'].map(get_separate_date_time)
all_transfers['dt_adm_number'] = all_transfers['dt_adm'].map(get_date_number)
...
#join onto dt_adm
all_transfers_with_edperf = all_transfers.join(ed_performance, on='dt_adm_number', how='left')
...
all_transfers_with_ed_beds= all_transfers_with_edperf.join(bedstate_info, on = 'dt_adm_number', how = 'left')


all_t_strain = all_transfers_with_ed_beds.drop(['transfer_date_number', 'dt_adm_number'], axis=1)
...
all_t_strain.to_csv('all_transfers_with_ed_perf_adm.csv', header=True, index=False)
