...
if it_is_weekend == True:
    for i in monthlist:
        ...
        month_data['is_weekend'] = month_data['transfer_dt'].map(is_weekend)
        month_weekend = month_data[month_data['is_weekend']]
        month_weekday = month_data[~month_data['is_weekend']]
        ...
        month_weekend_reduced = month_weekend.loc[month_weekend['age'] > 16].drop(['transfer_dt', 'dt_adm', 'dt_dis', 'spec', 'age', 'asa'], axis=1)
        get_network_analytics(month_weekend_reduced)
    ...
    for i in monthlist:
        ...
        number_of_transfers = len(month_data['transfer_month'])
        # drop the columns that are not needed for the graph, also select adults or children
        month_weekday_reduced = month_weekday.loc[month_weekday['age'] > 16].drop(['transfer_dt', 'dt_adm', 'dt_dis', 'spec', 'age', 'asa'], axis=1)
        get_network_analytics(month_weekday_reduced)
        print(i, number_of_transfers)
...
monthly_arima_df.to_csv('monthly_weekend_nov20.csv', header=True, index=False)
